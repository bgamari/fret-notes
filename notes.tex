...
intensities. Here, one measures $k_{R}$ and $k_\ET$ by observing
...
$n_D$ and $n_A$ for the acceptor and donor respectively. Assume
that the fluorescence of interest originates from donor decay pathway
$R_j$ and acceptor pathway $R_k$. We first note that not every
...
\begin{align*}
  n_D & = k_{exc} ~ \eta_D ~ \frac{k_{D-R_j}}{k_\ET + \sum k_{D-R_i} + \sum k_{D-\NR_i}} \\
      & = k_{exc} ~ \eta_D ~ \phi_D ~
...
\item[$k_D$] Rate of decay of fluorophore; includes both radiative and non-radiative processes. This is the rate of decay of fluorescence intensity $n_D(t) = e^{-t k_D}$.
\item[$k_R$] Rate of radiative decay of fluorophore. Equivalent radiative lifetime $\tau_R = k_R^{-1}$
